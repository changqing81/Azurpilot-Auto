(function () {
    ...
    function initChart() {
        ...
        (function () {
            for (var vi = 0; vi < visibleIndices.length; vi++) {
                ...
                if (mx === mn) { mx = mn + 100; }
                var rng = mx - mn;
                var extra = Math.max(rng * 0.2, Math.abs(mn) * 0.2, Math.abs(mx) * 0.1);
                mn -= extra;
                mx += extra;
        ...
        if (gridMin === Infinity) { gridMin = -100; gridMax = 100; }
        var gridRng = gridMax - gridMin;
        var gridExtra = Math.max(gridRng * 0.1, Math.abs(gridMin) * 0.08);
        gridMin -= gridExtra;
        gridMax += gridExtra;
    ...
    function renderZoomed() {
        ...
        // ---- 所有资源独立 Y 轴范围（缩放后，净变化可正可负，不钳制下限到 0）----
        var rightAxisCfgs = [];
        var yFns = {};
