...
def _args_has_shop_advanced() -> bool:
    """本地配置树是否包含 ShopAdvanced 商店策略组（上游功能，本地默认不带）。"""
    import json as _json

    args = _json.loads(Path('module/config/argument/args.json').read_text(encoding='utf-8'))
    return 'ShopAdvanced' in args.get('EventShop', {})
...
class ConfigApiTests(unittest.TestCase):
    ...
    @unittest.skipUnless(_args_has_shop_advanced(), '本地配置未启用 ShopAdvanced 商店策略功能')
