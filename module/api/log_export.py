"""日志导出工具：定位运行日志、打包错误日志目录、发放一次性下载令牌。

仅包含纯逻辑，由 module.api.router 的 WS 方法与 module.api.app 的 HTTP
下载端点调用，便于单独测试。
...
下载鉴权：WebSocket API 有密码保护，而浏览器下载走 HTTP GET。导出流程分两步：
先在已鉴权的 WS 会话里调用 `logs.requestExport`，服务端打包并签发一个
一次性、短时效的随机令牌；浏览器随后访问 `/api/v1/export/{token}` 取文件。
令牌用后即焚，无法被猜测或重放。
"""
...
# 导出体量上限：超过该原始字节数拒绝打包，避免远控链路上排队几分钟的请求
EXPORT_MAX_BYTES = 300 * 1024 * 1024
...
# 一次性下载令牌：token -> (路径, 建议文件名, 过期时间戳, 是否临时文件)
_DOWNLOAD_TOKENS: dict = {}
_TOKEN_LOCK = threading.Lock()
_TOKEN_TTL_SECONDS = 120


def register_download(path: Path, filename: str, temp_file: bool = False) -> str:
    """为待下载文件签发一次性令牌，返回可拼接进 URL 的 token。"""
    token = secrets.token_urlsafe(32)
    expires = time.time() + _TOKEN_TTL_SECONDS
    with _TOKEN_LOCK:
        # 顺带清理过期令牌，防止长期运行下字典缓慢膨胀
        expired = [key for key, value in _DOWNLOAD_TOKENS.items() if value[2] < time.time()]
        for key in expired:
            entry = _DOWNLOAD_TOKENS.pop(key)
            if entry[3]:
                Path(entry[0]).unlink(missing_ok=True)
        _DOWNLOAD_TOKENS[token] = (str(path), filename, expires, temp_file)
    return token


def pop_download(token: str):
    """取走令牌对应的文件；令牌一次性，取出即失效。

    Returns:
        (路径, 建议文件名, 是否临时文件)；令牌无效/过期返回 None。
        临时文件由调用方在响应发送完成后删除。
    """
    token = str(token or "")
    with _TOKEN_LOCK:
    ...
    if entry is None or entry[2] < time.time():
        if entry is not None and entry[3]:
            Path(entry[0]).unlink(missing_ok=True)
        return None
    return Path(entry[0]), entry[1], entry[3]


def get_project_root() -> Path:
    """项目根目录（含 module/、log/、config/）。

    本文件位于 module/api/ 下，故取 parents[2]；用绝对路径而不是相对
...
def build_error_log_zip(scope: str = SCOPE_FULL) -> Path:
    ...
    调用方负责在响应发送完成后删除该文件（见下载端点的 BackgroundTask）。
