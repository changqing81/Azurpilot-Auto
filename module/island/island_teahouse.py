...
class IslandTeahouse(IslandShopBase):
    ...
    def deduct_materials(self, product, number):
        ...
        合并冲突说明：原目标仓库使用 sunny_honey（阳光蜜水）逻辑，扣除 honey_lemon 库存；
        上游 #773 改为 strawberry_honey（草莓蜂蜜冰沙）逻辑，每份消耗 4 个蜂蜜。
        采纳上游新逻辑，因该修复更准确反映了实际配方；同时保留本地 _item_cn() 日志格式。
        """
        # 先调用父类方法扣除套餐原材料
    ...
    def apply_special_material_constraints(self, requirements):
        ...
        合并冲突说明：同上，移除旧版 sunny_honey 需求调整逻辑，
        保留 upstream strawberry_honey 逻辑，日志格式统一使用 _item_cn()。
        """
