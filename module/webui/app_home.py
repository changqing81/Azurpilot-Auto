...
class HomeMixin(WebUIMixinBase):
    ...
    def run(self, initial_page="home", localstorage=None) -> None:
        ...
        # 预载 CSS 失败校验。首屏样式经 HTTP <link> 预载，服务端因此跳过
        # WebSocket 注入；远控 P2P 弱网下主文档能成功而 CSS 子请求静默失败
        # （与视频 Range 请求失败同类问题），页面会永久裸奔成无样式白底。
        # 校验在 load 后 3s 进行（避免把"慢"误判为"失败"）：先自动重载一次
        # （60s 内仅一次，防循环），仍失败则经 WebSocket——已建立的可靠
        # 通道——触发服务端重注入全部样式，前端按 style id 去重补齐缺失项。
        if not getattr(self, "_css_verify_installed", False):
            self._css_verify_installed = True

            def _on_css_missing(_value):
                logger.warning("[WebUI] 检测到预载 CSS 加载失败，经 WebSocket 兜底重注入")
                load_webui_styles(is_mobile=self.is_mobile)

            put_input(name="__alas_css_missing", value="").style("display:none")
            pin_on_change(
                name="__alas_css_missing",
                onchange=_on_css_missing,
                clear=True,
                serial_mode=True,
            )
            run_js(
                """
                (function () {
                    if (window.__alasCssVerify) return;
                    window.__alasCssVerify = true;
                    function brokenLinks() {
                        var broken = [];
                        document.querySelectorAll('link[rel="stylesheet"]')
                            .forEach(function (l) {
                                try {
                                    if (!l.sheet) broken.push(l.href);
                                } catch (e) { /* 跨域 sheet 不可读，视为已加载 */ }
                            });
                        return broken;
                    }
                    function check() {
                        if (!brokenLinks().length) return;
                        var mark = 'alas-css-reload-at';
                        var now = Date.now();
                        var last = Number(sessionStorage.getItem(mark) || 0);
                        if (now - last > 60000) {
